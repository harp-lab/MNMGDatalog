...
# one representative dataset per task (road networks show the CPU-dominance effect)
...
def main():
    ...
    # column-width: ~3.3in wide, short
    fig, axes = plt.subplots(1, 2, figsize=(3.5, 2.4))
    x = np.arange(len(ENGINES))
    for ax, (task, ds, title) in zip(axes, PANELS):
        data = load(task)[ds]
        gpu = [data[e][0] for e in ENGINES]
        cpu = [data[e][1] for e in ENGINES]
        ax.bar(x, gpu, 0.7, color=GPU_C, edgecolor="black", linewidth=0.4, label="GPU")
        ax.bar(x, cpu, 0.7, bottom=gpu, color=CPU_C, edgecolor="black", linewidth=0.4, label="CPU")
        ax.set_xticks(x)
        ax.set_xticklabels([LABELS[e] for e in ENGINES], rotation=40, ha="right", fontsize=7)
        ax.set_title(title, fontsize=9)
        ax.tick_params(axis="y", labelsize=7)
        ax.grid(True, axis="y", linestyle="--", alpha=0.35)
        ax.set_axisbelow(True)
    axes[0].set_ylabel("Energy (J)", fontsize=8)
    axes[0].legend(loc="upper right", fontsize=7, frameon=True, handlelength=1.2)
